...
namespace TwelveMonthCalendar
{
    ...
    internal sealed class CalendarRefugeBehavior : CampaignBehaviorBase
    {
        ...
        internal bool TryEnterCompletedRefuge(out string failure)
        {
            ...
            {
                Diagnostics.Info("Refuge fixed scene profile rejected on entry. Scene="
                    + (sceneId ?? "<null>")
                    + "; SavedAccess=" + _state.WaterAccess
                    + "; SavedFort=" + (_state.FortPrefabId ?? "<null>")
                    + "; State=" + _state.State
                    + "; Detail=" + DescribeSavedSceneProfileFailure(sceneId, _state.FortPrefabId) + ".");
            ...
            bool isWinter = AgesOfCalradia.CalendarRefugeIntegration.IsWinter();
            ...
            out string reason)
        {
            ...
            if (!RefugeFortPrefabCatalog.IsAssetReady(fortPrefabId, out reason))
            {
                Diagnostics.Info("Refuge construction profile rejected: fort asset unavailable. Fort="
                    + (fortPrefabId ?? "<null>") + "; Reason=" + reason + ".");
                    ...
                    out sceneId))
            {
                ...
                {
                    Diagnostics.Info("Refuge scene profile is not ready; allowing configured test fallback. Climate="
                        + climate + "; Access=" + requestedAccess + "; Fort=" + fortPrefabId
                        + "; Detail=" + RefugeSceneProfileCatalog.DescribeProfileReadiness(climate, requestedAccess, fortPrefabId) + ".");
                    reason = string.Empty;
                    return true;
                }
                Diagnostics.Info("Refuge construction profile rejected: scene is not ready. Climate="
                    + climate + "; Access=" + requestedAccess + "; Fort=" + fortPrefabId
                    + "; Detail=" + RefugeSceneProfileCatalog.DescribeProfileReadiness(climate, requestedAccess, fortPrefabId) + ".");
        ...
        private static string DescribeSavedSceneProfileFailure(string sceneId, string fortPrefabId)
        {
            RefugeSceneProfile profile;
            if (!RefugeSceneProfileCatalog.TryGetProfile(sceneId, out profile))
            {
                return "scene ID is not registered in the refuge profile catalog";
            }

            if (!string.Equals(profile.FortPrefabId, fortPrefabId, StringComparison.Ordinal))
            {
                return "saved fort does not match profile fort; profile fort=" + profile.FortPrefabId;
            }

            return RefugeSceneProfileCatalog.DescribeProfileReadiness(
                profile.Climate,
                profile.WaterAccess,
                profile.FortPrefabId);
        }
