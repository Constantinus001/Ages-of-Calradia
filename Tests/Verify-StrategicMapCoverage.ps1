...
Assert-True ($ledgerSource -match 'rgba\.Substring\(0, 6\) \+ "88"') 'Contested province fills must be transparent so ownership borders remain readable.'
...
Assert-True ($worldCalendarPrefab -match '<TextureWidget[^>]*TextureProviderName="CalendarStrategicMapTextureProvider"') 'The World Calendar prefab is not using the composed Strategic Map texture provider.'
...
Assert-True ($textureProviderSource -match 'class CalendarStrategicMapTextureProvider : TextureProvider') 'The Strategic Map composer is not a Gauntlet texture provider.'
...
Assert-True ($textureProviderSource -match 'Color\.FromArgb\(255, 170, 176, 182\)') 'The Strategic Map town/castle markers are not filled neutral gray.'
